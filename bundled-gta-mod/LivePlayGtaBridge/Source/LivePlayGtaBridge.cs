...
public class LivePlayGtaBridge : Script
{
    ...
    private DateTime _nightVisionUntil = DateTime.MinValue;
    private DateTime _heatVisionUntil = DateTime.MinValue;
    private DateTime _radarHiddenUntil = DateTime.MinValue;
    private DateTime _hudHiddenUntil = DateTime.MinValue;
    private DateTime _blackoutUntil = DateTime.MinValue;
    private DateTime _invincibleUntil = DateTime.MinValue;
    private DateTime _slowMotionUntil = DateTime.MinValue;
    private bool _nightVisionActive = false;
    private bool _heatVisionActive = false;
    private bool _radarHiddenActive = false;
    private bool _hudHiddenActive = false;
    private bool _blackoutActive = false;
    private bool _invincibleActive = false;
    ...
    private void LoadConfig()
    {
        try
        {
            ...
            int parsedPort;
            if (port.Success && int.TryParse(port.Groups[1].Value, out parsedPort) && parsedPort > 0) _port = parsedPort;
        }
        catch { }
    }
    ...
    private void ExecuteLivePlayCommand(string command)
    {
        ...
        if (normalized.StartsWith("blackout_on")) { StartBlackoutEffect(12000); Notify("LivePlay: blackout por 12s"); return; }
        if (normalized.StartsWith("blackout_off")) { StopBlackoutEffect(); Notify("LivePlay: blackout off"); return; }
        if (normalized.StartsWith("night_vision")) { StartNightVisionEffect(12000); Notify("LivePlay: visão noturna por 12s"); return; }
        if (normalized.StartsWith("heat_vision")) { StartHeatVisionEffect(12000); Notify("LivePlay: visão térmica por 12s"); return; }
        if (normalized.StartsWith("no_hud")) { StartHudHiddenEffect(12000); Notify("LivePlay: HUD oculto por 12s"); return; }
        if (normalized.StartsWith("no_radar")) { StartRadarHiddenEffect(12000); Notify("LivePlay: radar oculto por 12s"); return; }
    ...
    private void ExecuteLivePlayEffectSlug(string slug)
    {
        ...
        // Visuais temporários precisam ser tratados antes de clima/tempo, porque night_vision contém "night".
        if (slug.Contains("hud") || slug.Contains("radar") || slug.Contains("phone") || slug.Contains("screen") || slug.Contains("vision") || slug.Contains("camera") || slug.Contains("pitch") || slug.Contains("fov") || slug.Contains("colors") || slug.Contains("noire") || slug.Contains("textureless") || slug.Contains("potato") || slug.Contains("static") || slug.Contains("gamespeed") || slug.Contains("slow_motion")) { VisualEffect(slug); return; }
    ...
    {
        StartInvincibleEffect(10000);
        Notify("LivePlay GTA: invencível por 10s");
    }
    ...
    {
        if (slug.Contains("blackout"))
        {
            if (slug.Contains("off"))
            {
                StopBlackoutEffect();
                Notify("LivePlay GTA: blackout off");
            }
            else
            {
                StartBlackoutEffect(12000);
                Notify("LivePlay GTA: blackout por 12s");
            }
            return;
        }
    ...
    private void VisualEffect(string slug)
    {
        ...
        if (slug.Contains("night_vision"))
        {
            StartNightVisionEffect(12000);
            handled = true;
        }

        if (slug.Contains("heat_vision"))
        {
            StartHeatVisionEffect(12000);
            handled = true;
        }

        if (slug.Contains("hud"))
        {
            StartHudHiddenEffect(12000);
            handled = true;
        }

        if (slug.Contains("radar"))
        {
            StartRadarHiddenEffect(12000);
            handled = true;
        }

        if (slug.Contains("gamespeed") || slug.Contains("slow_motion"))
        {
            StartSlowMotionEffect(slug.Contains("0_2") || slug.Contains("x0_2") ? 0.2f : 0.5f, 10000);
            handled = true;
        }

        Function.Call(Hash.SHAKE_GAMEPLAY_CAM, slug.Contains("spinning") ? "DRUNK_SHAKE" : "SMALL_EXPLOSION_SHAKE", 0.8f);
        Notify("LivePlay GTA: " + Pretty(slug) + (handled ? " temporário" : ""));
    }
    ...
    private void UpdateTemporaryEffects()
    {
        ...
        if (_nightVisionActive && now >= _nightVisionUntil) StopNightVisionEffect();
        if (_heatVisionActive && now >= _heatVisionUntil) StopHeatVisionEffect();
        if (_radarHiddenActive && now >= _radarHiddenUntil) StopRadarHiddenEffect();
        if (_hudHiddenActive && now >= _hudHiddenUntil) StopHudHiddenEffect();
        if (_blackoutActive && now >= _blackoutUntil) StopBlackoutEffect();
        if (_invincibleActive && now >= _invincibleUntil) StopInvincibleEffect();
        if (_slowMotionActive && now >= _slowMotionUntil) StopSlowMotionEffect();

        // DISPLAY_HUD/DISPLAY_RADAR precisam ser reaplicados enquanto o efeito estiver ativo,
        // senão o GTA pode reexibir em alguns frames/menus.
        if (_radarHiddenActive) Function.Call(Hash.DISPLAY_RADAR, false);
        if (_hudHiddenActive) Function.Call(Hash.DISPLAY_HUD, false);
    }

    private void StartNightVisionEffect(int durationMs)
    {
        _nightVisionActive = true;
        _nightVisionUntil = DateTime.UtcNow.AddMilliseconds(durationMs);
        Function.Call(Hash.SET_NIGHTVISION, true);
    }

    private void StopNightVisionEffect()
    {
        _nightVisionActive = false;
        _nightVisionUntil = DateTime.MinValue;
        Function.Call(Hash.SET_NIGHTVISION, false);
    }

    private void StartHeatVisionEffect(int durationMs)
    {
        _heatVisionActive = true;
        _heatVisionUntil = DateTime.UtcNow.AddMilliseconds(durationMs);
        Function.Call(Hash.SET_SEETHROUGH, true);
    }

    private void StopHeatVisionEffect()
    {
        _heatVisionActive = false;
        _heatVisionUntil = DateTime.MinValue;
        Function.Call(Hash.SET_SEETHROUGH, false);
    }

    private void StartRadarHiddenEffect(int durationMs)
    {
        _radarHiddenActive = true;
        _radarHiddenUntil = DateTime.UtcNow.AddMilliseconds(durationMs);
        Function.Call(Hash.DISPLAY_RADAR, false);
    }

    private void StopRadarHiddenEffect()
    {
        _radarHiddenActive = false;
        _radarHiddenUntil = DateTime.MinValue;
        Function.Call(Hash.DISPLAY_RADAR, true);
    }

    private void StartHudHiddenEffect(int durationMs)
    {
        _hudHiddenActive = true;
        _hudHiddenUntil = DateTime.UtcNow.AddMilliseconds(durationMs);
        Function.Call(Hash.DISPLAY_HUD, false);
    }

    private void StopHudHiddenEffect()
    {
        _hudHiddenActive = false;
        _hudHiddenUntil = DateTime.MinValue;
        Function.Call(Hash.DISPLAY_HUD, true);
    }

    private void StartBlackoutEffect(int durationMs)
    {
        _blackoutActive = true;
        _blackoutUntil = DateTime.UtcNow.AddMilliseconds(durationMs);
        Function.Call(Hash.SET_ARTIFICIAL_LIGHTS_STATE, true);
    }

    private void StopBlackoutEffect()
    {
        _blackoutActive = false;
        _blackoutUntil = DateTime.MinValue;
        Function.Call(Hash.SET_ARTIFICIAL_LIGHTS_STATE, false);
    }

    private void StartInvincibleEffect(int durationMs)
    {
        _invincibleActive = true;
        _invincibleUntil = DateTime.UtcNow.AddMilliseconds(durationMs);
        Game.Player.Character.IsInvincible = true;
    }

    private void StopInvincibleEffect()
    {
        _invincibleActive = false;
        _invincibleUntil = DateTime.MinValue;
        try { Game.Player.Character.IsInvincible = false; } catch { }
    }

    private void StartSlowMotionEffect(float scale, int durationMs)
    {
        _slowMotionActive = true;
        _slowMotionUntil = DateTime.UtcNow.AddMilliseconds(durationMs);
        Function.Call(Hash.SET_TIME_SCALE, scale);
    }

    private void StopSlowMotionEffect()
    {
        _slowMotionActive = false;
        _slowMotionUntil = DateTime.MinValue;
        Function.Call(Hash.SET_TIME_SCALE, 1.0f);
    }

    private void ResetAllTemporaryEffects()
    {
        StopNightVisionEffect();
        StopHeatVisionEffect();
        StopRadarHiddenEffect();
        StopHudHiddenEffect();
        StopBlackoutEffect();
